<?php
include_once('config.php');
session_start();

if (isset($_SESSION['PHONE'])) {
    $phone = $_SESSION['PHONE'];
$id=$_GET["id"];
// $name=$_GET["name"];

    ?>
<!Doctype html>
<html>
<head>
<title>MEDCARE</title>
<link rel="apple-touch-icon" sizes="180x180" href="apple-touch-icon.png">
<link rel="icon" type="image/png" sizes="32x32" href="favicon-32x32.png">
<link rel="icon" type="image/png" sizes="16x16" href="favicon-16x16.png">
<link rel="manifest" href="site.webmanifest">
    <meta name="viewport" content="width=device-width, intial-scale=1.0" />
    <link rel="stylesheet" href="css/reports.css" />
    <link rel="stylesheet" href="css/bootstrap.min.css" />
<link rel="stylesheet" type="text/css" href="fonts/font-awesome-4.7.0/css/font-awesome.min.css">
<script src="file/jquery-3.3.1.js"></script>
<script src="file/popper.min.js"></script>
    <style>
  
    </style>
    
    </head>
    <body>


    <!-- <div class="imgGallery">
 
</div> -->


    <section class="container-fluid register-form">
        
        <div class="row">
        
            <div class="col-lg-4 col-xl-4 col-md-6 col-12 col-sm-12 mx-auto">
            
                <div class="page-wrap">
                    <h1 class="text-center">Add Reports</h1>
                    <p>Save your information! Protect your life!</p>
  
                    <form class="p-2" id="submitForm">

                    <input type="text" class="d-none" value="<?php echo $id; ?>" name="id" />
                  

         <div class="styled-input">
      <input type="text" class="name" name="name" value="<?php echo $name; ?>" disabled placeholder="" required />
      <!-- <label>Name</label> -->
      <span></span>
    </div>
        <!-- <div class="styled-input">
      <input type="text" class="" placeholder="" required />
      <label>Name</label>
      <span></span>
    </div> -->
     <div class="styled-input ">
           <select class="form-control" name="type" id="type" required id="relation">
           <option>Select Report Type</option>
            <option>Hopital all reports</option>
               <option>Clinical all reports</option>
               <option>Bills</option>
               <option>Lab reports</option>
               <option>X-ray reports</option>
               <option>Test reports</option>
               <option>Prescriptions</option>
               <option>Certificates</option>
              <option>Other</option>
           </select>
    </div>
        <div class="styled-input">
      <input type="date" name="date" id="date" class="" required />
      <label class="w-100" style="background:#fff;">Select Date</label>
      <span></span>
    </div>
       <div class="styled-input">
       <input type="file" name="files[]" required class="" multiple>
      <label class="w-100" style="background:#fff;">Select Images</label>
      <span class=></span>      
    </div>
    <h6>* Maximum upload images 20</h6>


       <div class="styled-input">
           <textarea class="" name="description" required rows="4" placeholder=""></textarea>
           <label class="">Description</label>
      <!-- <span></span> -->
    </div>
    <div class="my-3">
      <button type="submit" style="border-radius:5px !important;">Add Report</button>
      </div>
  </form>
</div>
            <div class="p-2 m-0 text-center" id="login_p">
                <span class="">Goto <a href="index1.php">Home</a></span>
                </div>
            </div>
        
        </div>
        
        
        </section>



              <script type="text/javascript">
  $(document).ready(function(){
    $("#submitForm").on("submit", function(e){
      e.preventDefault();
      var formData = $(this).serialize();
    var dob=$("#date").val();
    var type=$("#type").val();
var d=new Date();
var dt=d.getFullYear();
var md=new Date(dob);
var mdt=md.getFullYear();

var mon=d.getMonth();
var mon1=md.getMonth();

var date=d.getDate();
var date1=md.getDate();

	if(type === "Select Report Type")
	{
		alert("Select report type");
	}
	else
	{
    if((mdt > 1950 && mdt <= dt) && (mon >= mon1) && (date >= date1))
    {
      $.ajax({
        url  : "report_store.php",
        type : "POST",
        contentType: false,
        processData:false,
        cache:false,
        data : new FormData(this),
        success:function(result){
          // alert(result);
          if (result == "yes") {
            alert("Report stored sucessfully");
            window.location ='index1.php';          
          }
		  else if(result == "no"){
            alert("Report not stored");
            alert(result);
          }     
		  else{
			  alert(result);
		  }     
        }
      }); 
    }
    else{
  alert("Enter correct Date");
}
	} 
      });    
  });
</script>


    </body>
</html>


<?php

}else{
	header("Location:login.php");
	die();
}

?>